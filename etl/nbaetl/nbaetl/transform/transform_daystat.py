...
from ..utils import (
    add_prefix, daterange, daystat_name,
    daystat_name_transformed, daystat_name_filled,
    gamelog_name,
)
        ...
        for season_type in ["playoff", "regular"]:
            ...
            for i, cur_date in enumerate(daterange(start_date, end_date)):
                ...
                for lag_len in lag_len_arr:
                    df_file_name = daystat_name(stat_type, year, season_type, cur_date, lag_len)
                    df_file_path = f'{file_config.ROOT_RAW}/{data_dir}/{year}/{df_file_name}'
                    ...
                    df_daystat_lagged = pd.read_csv(df_file_path)
            ...
            stat_name = daystat_name_transformed(stat_type, year, season_type)
            df_daystat_transformed.to_csv(
                f"{file_config.ROOT_TRANSFORMED}/{data_dir}/{stat_name}",
                index=False
            )
...
def fillpast_daystat(
    ...
    for year in year_arr:
        stat_regular_name = daystat_name_transformed(stat_type, year, 'regular')
        stat_playoff_name = daystat_name_transformed(stat_type, year, 'playoff')
        stat_regular_filled = daystat_name_filled(stat_type, year, 'regular')
        stat_playoff_filled = daystat_name_filled(stat_type, year, 'playoff')

        current_daystat_regular = pd.read_csv(
            f"{file_config.ROOT_TRANSFORMED}/{file_config.TEAMSTAT}/{stat_regular_name}"
        )
        current_daystat_playoff = pd.read_csv(
            f"{file_config.ROOT_TRANSFORMED}/{file_config.TEAMSTAT}/{stat_playoff_name}"
        )
        filled_daystat_regular = fill_past_teamstat_regular(
            current_daystat_regular, year, stat_type, file_config,
        )
        ...
        filled_daystat_regular.to_csv(f"{file_config.ROOT_TRANSFORMED}/{data_dir}/{stat_regular_filled}", index=False)
        filled_daystat_playoff.to_csv(f"{file_config.ROOT_TRANSFORMED}/{data_dir}/{stat_playoff_filled}", index=False)
...
def fill_past_teamstat_regular(
    ...
    # dont change it
    prev_year_name = daystat_name_transformed(stat_type, current_year-1, 'regular')
    if not os.path.exists(f"{file_config.ROOT_TRANSFORMED}/{file_config.TEAMSTAT}/{prev_year_name}"):
        return current_year_stat

    current_gamelog = pd.read_csv(
        f"{file_config.ROOT_TRANSFORMED}/{file_config.GAMELOG}/{gamelog_name(current_year, 'regular')}"
    )
    prev_year_stat = pd.read_csv(
        f"{file_config.ROOT_TRANSFORMED}/{file_config.TEAMSTAT}/{prev_year_name}"
    )
