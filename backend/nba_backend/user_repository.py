...
from nba_backend.utils.database_utils import CommitMode, managed_tx_method
...
class UserRepository:
    ...
    def get_user(self, id: UUID, api_client_id: Optional[UUID] = None) -> User:
        ...
        Raises:
            NBAError: 403 if untrusted client attempts to query foreign users. 404 if user with ID not found.
        """
        ...
        if user is None:
            raise NBAError(
                "User not found", NBAErrorCode.USER_NOT_FOUND, HTTP_404_NOT_FOUND
            )
    ...
    def update_user(
        ...
        Raises:
            NBAError: 403 if untrusted client attempts to update a user. 404 if user with ID not found.
        """
    ...
    def mark_user_deleted(self, id: UUID) -> None:
        ...
        Raises:
            NBAError: 403 if untrusted client attempts to delete a user. 404 if user with ID not found.
        """
        if not self.api_client.trusted:
            raise NBAError(
                "Forbidden", NBAErrorCode.API_CLIENT_NOT_AUTHORIZED, HTTP_403_FORBIDDEN
            ...
            if create_missing:
                ...
                if auth_method == "system":
                    user.show_on_leaderboard = (
                        False  # don't show system users, e.g. import user
                    )
                    user.tos_acceptance_date = utcnow()
                self.db.add(user)
    ...
    def lookup_client_user(
        self, client_user: protocol_schema.User, create_missing: bool = True
    ) -> User | None:
        if not client_user:
            return None
        ...
        if not (client_user.auth_method and client_user.id):
            raise NBAError(
                "Auth method or username missing.",
                NBAErrorCode.AUTH_AND_USERNAME_REQUIRED,
            )
    ...
    @managed_tx_method(CommitMode.COMMIT)
    def lookup_system_user(
        self, username: str, create_missing: bool = True
    ) -> User | None:
        return self._lookup_user_tx(
            username=username,
            auth_method="system",
    ...
    def query_users_ordered_by_username(
        ...
        if gte_username is not None:
            if gt_id:
                qry = qry.filter(
                    or_(
                        User.username > gte_username,
                        and_(User.username == gte_username, User.id > gt_id),
                    )
                )
            else:
        ...
        if lte_username is not None:
            if lt_id:
                qry = qry.filter(
                    or_(
                        User.username < lte_username,
                        and_(User.username == lte_username, User.id < lt_id),
                    )
                )
            else:
