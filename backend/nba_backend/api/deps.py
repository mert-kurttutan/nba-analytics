...
oasst_user_query = APIKeyQuery(
    name="oasst_user", scheme_name="oasst-user", auto_error=False
)
oasst_user_header = APIKeyHeader(
    name="x-oasst-user", scheme_name="oasst-user", auto_error=False
)
...
def api_auth(
    ...
    raise NBAError(
        ...
        error_code=NBAErrorCode.API_CLIENT_NOT_AUTHORIZED,
...
def get_root_token(
    bearer_token: HTTPAuthorizationCredentials = Security(bearer_token),
) -> str:
    if bearer_token:
    ...
    raise NBAError(
        "Could not validate credentials",
        error_code=NBAErrorCode.ROOT_TOKEN_NOT_AUTHORIZED,
        http_status_code=HTTPStatus.FORBIDDEN,
    )
